...
Выберите или создайте **папку сессии** — в этой папке будут храниться все файлы.
...
# Результат: ./BacteriaAnalyzer (~120 МБ, standalone)
```

### Структура бинарника

- Включает пакеты: `analysis`, `ui`, `utils`, `labeling`, `testing`
- Исключены: `train`, `.venv`, `.venv-dev`, `test_images/`
...
|---|---|---|---|
| `.venv` (runtime) | `uv sync` | ~600 МБ | Запуск приложения, Nuitka-сборка, тестирование |
| `.venv-dev` (dev) | `UV_PROJECT_ENVIRONMENT=.venv-dev uv sync --extra dev` | ~5 ГБ | Разработка, обучение U-Net, аугментация |
...
- Два виртуальных окружения: `.venv` (runtime) и `.venv-dev` (dev с torch).
