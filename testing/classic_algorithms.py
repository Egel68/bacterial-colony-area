...
class _ClassicBase(BaseDetectionAlgorithm):
    ...
    def detect(self, image: np.ndarray, is_cropped: bool = False) -> np.ndarray:
        ...
        colony_mask, _ = self.detector.detect_colonies(
            image, petri_mask, params=self.params, petri_info=petri_info,
        )
        return colony_mask
...
class ClassicDefault(_ClassicBase):
    ...
    params = AnalysisParams(
        sensitivity=0.5, min_colony_size=50, margin_percent=10,
        contrast=1.0, solid_fill=False, fill_strength=15,
    )
...
class ClassicHighSensitivity(_ClassicBase):
    ...
    params = AnalysisParams(
        sensitivity=0.7, min_colony_size=30, margin_percent=5,
        contrast=1.0, solid_fill=False, fill_strength=15,
    )
...
class ClassicSolidFill(_ClassicBase):
    ...
    params = AnalysisParams(
        sensitivity=0.3, min_colony_size=50, margin_percent=15,
        contrast=1.0, solid_fill=True, fill_strength=15,
    )
...
class ClassicLowSensitivity(_ClassicBase):
    ...
    params = AnalysisParams(
        sensitivity=0.3, min_colony_size=100, margin_percent=10,
        contrast=1.0, solid_fill=False, fill_strength=15,
    )
