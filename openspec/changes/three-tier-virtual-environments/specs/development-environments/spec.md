...
## Purpose
Трёхуровневая структура виртуальных окружений проекта: `.venv` (runtime), `.venv-dev` (разработка без ML), `.venv-full` (полная разработка с обучением NN).
...
#### Scenario: Build env does not leak build tools into runtime env
- **КОГДА** выполнен `scripts/build_nuitka.sh`
- **ТОГДА** в `.venv` SHALL NOT появиться `nuitka`, `zstandard` (они устанавливаются только в `.venv-build`)
...
#### Scenario: Build env excluded from git
- **КОГДА** выполняется `git status`
- **ТОГДА** каталог `.venv-build/` SHALL NOT отображаться как untracked
