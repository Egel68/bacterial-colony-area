...
def _build_comparison_sections(
    ...
    """Строит HTML-секции «Сравнение алгоритмов» по результату compare_algorithms."""
    sections = ""
...
def generate_report(
    ...
    """Генерирует HTML-отчёт со сводной таблицей, графиками и (опционально) сравнением.

    include_per_snapshot=False скрывает детальные per-algo таблицы (по снимкам).
    comparison — результат testing.runner.compare_algorithms: добавляет таблицу
    «победитель по снимку» для каждой метрики.
    """
