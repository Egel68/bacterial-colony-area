...
class TestMargin:
    def test_margin_reduces_working_area(
        self, binary_mask_circle, petri_mask, petri_info
    ):
        calc = make_calculator()
        res_no_margin = calc.calculate_areas(
            petri_mask, binary_mask_circle, petri_info, margin_percent=0
        )
        res_with_margin = calc.calculate_areas(
            petri_mask, binary_mask_circle, petri_info, margin_percent=10
        )
...
def test_coverage_bounds_property(col_w, col_h):
    ...
    info = PetriInfo(cx=100, cy=100, radius=80, image_shape=(200, 200))
