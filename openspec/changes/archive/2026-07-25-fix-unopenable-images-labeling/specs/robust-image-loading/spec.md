...
Система SHALL поддерживать как минимум следующие форматы: PNG, JPEG, JPG, BMP, TIFF, TIF, WebP. Дополнительные форматы, поддерживаемые OpenCV (например, PXM, JPEG 2000), SHALL также работать через путь fallback.
